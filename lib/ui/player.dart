...
class _PlayerPageState extends State<PlayerPage> {
  ...
  @override
  void dispose() { SystemChrome.setPreferredOrientations([DeviceOrientation.portraitUp]); SystemChrome.setEnabledSystemUIMode(SystemUiMode.edgeToEdge); _controller?.dispose(); _hideTimer?.cancel(); super.dispose(); }
  ...
  Future<void> _toggleFavorite() async {
    if (isFavorite) { await DatabaseHelper().removeFromFavorites(widget.id); } 
      ...
      await DatabaseHelper().addToFavorites({'drama_id': widget.id, 'drama_title': widget.title, 'drama_poster': dramaData?['cover'], 'total_episodes': dramaData?['total_episodes'] ?? totalEpisodes, 'platform': widget.source, 'added_at': DateTime.now().millisecondsSinceEpoch});
    }
  ...
  void _initDefaultSubtitle() {
    ...
    Map? sBox;
    for (var s in subtitles) { if (s['language'].toString().toLowerCase().contains('id')) { sBox = s; break; } }
    if (sBox != null) {
      bool skip = widget.title.toLowerCase().contains('sulih suara') || currentAudio.toLowerCase().contains('indo');
      setState(() { currentSubtitle = "Indonesia"; isSubtitleOn = !skip; });
      _fetchSubtitleFile(sBox['url']);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget overlayControls = Stack(children: [
      ...
      if (showControls && !isLoading) ...[
        Positioned(top: 0, left: 0, right: 0, child: Container(padding: const EdgeInsets.only(top: 30, left: 16, right: 16, bottom: 16), decoration: const BoxDecoration(gradient: LinearGradient(colors: [Colors.black54, Colors.transparent], begin: Alignment.topCenter, end: Alignment.bottomCenter)), child: Row(children: [IconButton(icon: const Icon(Icons.arrow_back, color: Colors.white), onPressed: () => isFullMode ? _toggleFullscreen() : Navigator.pop(context)), const SizedBox(width: 8), Expanded(child: Text("${widget.title} - Ep $currentEpisode", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)), TextButton(onPressed: () => _showQualitySelection(false), child: Text(currentQuality, style: const TextStyle(color: Color(0xFF06B6D4), fontWeight: FontWeight.bold)))]))),
        Center(child: Row(mainAxisSize: MainAxisSize.min, children: [IconButton(icon: const Icon(Icons.replay_10, color: Colors.white, size: 30), onPressed: _skipBackward), const SizedBox(width: 40), ElevatedButton(onPressed: _togglePlay, style: ElevatedButton.styleFrom(shape: const CircleBorder(), backgroundColor: const Color(0xFF06B6D4), padding: const EdgeInsets.all(12)), child: Icon(_controller != null && _controller!.value.isPlaying ? Icons.pause : Icons.play_arrow, color: Colors.white, size: 32)), const SizedBox(width: 40), IconButton(icon: const Icon(Icons.forward_10, color: Colors.white, size: 30), onPressed: _skipForward)])),
        Positioned(bottom: 0, left: 0, right: 0, child: Container(padding: const EdgeInsets.all(12), decoration: const BoxDecoration(gradient: LinearGradient(colors: [Colors.black87, Colors.transparent], begin: Alignment.bottomCenter, end: Alignment.topCenter)), child: Column(mainAxisSize: MainAxisSize.min, children: [
          Row(children: [Text(_formatDuration(_position), style: const TextStyle(color: Colors.white, fontSize: 11)), Expanded(child: Slider(value: _position, max: _duration > 0 ? _duration : 1, activeColor: const Color(0xFF06B6D4), inactiveColor: Colors.white24, onChanged: _seekTo)), Text(_formatDuration(_duration), style: const TextStyle(color: Colors.white, fontSize: 11))]),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Row(children: [IconButton(icon: const Icon(Icons.skip_previous, color: Colors.white, size: 20), onPressed: _prevEpisode), IconButton(icon: const Icon(Icons.skip_next, color: Colors.white, size: 20), onPressed: _nextEpisode)]),
            Row(children: [TextButton(onPressed: () => _showQualitySelection(false), child: Text(currentQuality, style: const TextStyle(color: Color(0xFF06B6D4), fontSize: 12, fontWeight: FontWeight.bold))), IconButton(icon: const Icon(Icons.settings, color: Colors.white, size: 20), onPressed: _showMainSettingsDialog), IconButton(icon: Icon(isFullMode ? Icons.fullscreen_exit : Icons.fullscreen, color: Colors.white, size: 20), onPressed: _toggleFullscreen)]),
          ])
        ])))
      ]
    ]);

    if (isFullMode) return Scaffold(backgroundColor: Colors.black, body: Stack(children: [videoWidget, subOverlay, Positioned.fill(child: overlayControls)]));

    return Scaffold(
      backgroundColor: const Color(0xFF0D1117),
      body: SafeArea(
        child: Column(children: [
          Container(height: 340, color: Colors.black, child: Stack(children: [videoWidget, subOverlay, Positioned.fill(child: overlayControls)])),
          Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8), Text(widget.title, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8), if (dramaData != null) Text(dramaData!['description'] ?? '', style: const TextStyle(color: Colors.white70, fontSize: 12, height: 1.4), maxLines: 3, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: GestureDetector(onTap: _toggleFavorite, child: Container(height: 48, decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF3B82F6), Color(0xFF06B6D4)]), borderRadius: BorderRadius.circular(24)), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(isFavorite ? Icons.favorite : Icons.favorite_border, color: Colors.white), const SizedBox(width: 8), const Text("Favorit", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))])))),
              const SizedBox(width: 12),
              Expanded(child: GestureDetector(onTap: () => Navigator.pushNamed(context, '/download'), child: Container(height: 48, decoration: BoxDecoration(color: const Color(0xFF1E293B), borderRadius: BorderRadius.circular(24), border: Border.all(color: const Color(0xFF06B6D4).withOpacity(0.5))), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.download, color: Color(0xFF06B6D4)), SizedBox(width: 8), Text("Download", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))]))))
            ]),
            const SizedBox(height: 24), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("DAFTAR EPISODE", style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)), Text("$totalEpisodes Ep", style: const TextStyle(color: Colors.white38, fontSize: 12))]),
            const SizedBox(height: 12),
            GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 5, childAspectRatio: 1.0, crossAxisSpacing: 10, mainAxisSpacing: 10), itemCount: totalEpisodes, itemBuilder: (context, index) {
              final epNum = index + 1; final isCurrent = epNum == currentEpisode;
              return GestureDetector(onTap: () => _loadVideo(epNum), child: Container(decoration: BoxDecoration(color: isCurrent ? Colors.transparent : const Color(0xFF1E293B), borderRadius: BorderRadius.circular(12), border: Border.all(color: isCurrent ? const Color(0xFF06B6D4) : Colors.transparent, width: 2)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text("EPISODE", style: TextStyle(color: isCurrent ? const Color(0xFF06B6D4) : Colors.white38, fontSize: 8, fontWeight: FontWeight.bold)), const SizedBox(height: 2), Text(isCurrent ? "DIPUTAR\n$epNum" : "$epNum", textAlign: TextAlign.center, style: TextStyle(color: isCurrent ? const Color(0xFF06B6D4) : Colors.white, fontSize: isCurrent ? 9 : 12, fontWeight: FontWeight.bold))])));
            })
          ])))
        ]),
      ),
