...
class _PlayerPageState extends State<PlayerPage> {
  ...
  Future<void> _loadVideo(int episodeNum, {Duration? startPosition}) async {
    setState(() { isLoading = true; currentEpisode = episodeNum; _parsedSubtitles.clear(); _currentSubtitleText = ""; });
    ...
    if (res != null && res['success'] == true && res['data'] != null) {
      ...
      if (qualities.isNotEmpty) {
        for (var q in qualities) { 
          ...
          if (qLabel == currentQuality.toLowerCase() || (currentQuality.toLowerCase() == "auto" && qLabel == "auto")) { 
            videoUrl = q['url'] ?? ''; 
            break; 
          } 
        }
      ...
      if (videoUrl.isNotEmpty) {
        ...
        _controller!.addListener(() {
          if (mounted) {
            ...
            setState(() { 
              _position = currentPos.inSeconds.toDouble(); 
              _duration = _controller!.value.duration.inSeconds.toDouble(); 
              _updateSubtitleText(currentPos);
            });
          }
  ...
  Future<void> _fetchSubtitleFile(String url) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() { _parsedSubtitles = tempSubs; });
      }
    } catch (_) { setState(() { _parsedSubtitles = []; }); }
  }
  ...
  void _updateSubtitleText(Duration currentPosition) {
    if (_parsedSubtitles.isEmpty || !isSubtitleOn) { _currentSubtitleText = ""; return; }
    String foundText = "";
        ...
        content: Column(
          ...
          children: [
            ListTile(
              ...
              trailing: currentQuality.toLowerCase() == "auto" ? const Icon(Icons.check, color: Color(0xFF06B6D4)) : null,
              onTap: () async {
            ...
            if (qualities.isEmpty)
              const Padding(padding: EdgeInsets.all(12), child: Text("Resolusi manual tidak dikirim dari API", style: TextStyle(color: Colors.black38, fontSize: 12)))
            else
              ...qualities.map((q) {
                final quality = q['quality'] ?? 'Auto';
                if (quality.toString().toLowerCase() == "auto") return const SizedBox.shrink();
                return ListTile(
                  title: Text(quality.toString().toUpperCase(), style: const TextStyle(color: Colors.black87)),
                  trailing: currentQuality.toLowerCase() == quality.toString().toLowerCase() ? const Icon(Icons.check, color: Color(0xFF06B6D4)) : null,
                  onTap: () async {
        ...
        content: Column(
          ...
          children: audioTracks.map((a) {
            ...
            return ListTile(
              ...
              onTap: () { setState(() => currentAudio = lang); Navigator.pop(context); },
            );
          }).toList(),
        ),
      ),
